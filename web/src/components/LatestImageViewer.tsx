...
interface LatestImageViewerProps {
  /** API path (without /api/v2 prefix), e.g. "/latest-image" */
  endpoint: string;
  /** React Query key for this latest-image query */
  queryKey: string;
  emptyLabel?: string;
}
...
export default function LatestImageViewer({
  endpoint,
  queryKey,
  emptyLabel = 'No image available',
}: LatestImageViewerProps) {
  ...
  const imageQ = useQuery({
    queryKey: [queryKey, cameraId],
    queryFn: () =>
      api<LatestImage>(`${endpoint}?camera_id=${cameraId}&limit_s=900&night=1`),
          ...
          {noCameras ? (
            <div className="aspect-video w-full max-w-3xl border border-danger/40 rounded-lg flex items-center justify-center bg-danger/5">
              <div className="text-center space-y-2 px-4">
                <div className="text-danger text-base font-medium">No cameras connected</div>
                <div className="text-ink-dim text-sm max-w-md">
                  No cameras have been registered yet. Connect a camera and start the indi-allsky service, then refresh.
                </div>
              </div>
            </div>
          ) : url ? (
            ...
            <div className="aspect-video w-full max-w-3xl border border-edge rounded-lg flex items-center justify-center bg-bg-1/30">
              <div className="text-center space-y-1">
                ...
                {!loading && (
                  <div className="text-ink-dim/60 text-xs">Capture may be paused or down</div>
                )}
              </div>
            </div>
          )}
