...
interface LoopViewerProps {
  /** API path (without /api/v2 prefix), e.g. "/loop" */
  endpoint: string;
  /** React Query key root for this viewer */
  queryKey: string;
  /** localStorage key for saved settings (unique per viewer) */
  settingsKey: string;
}
...
function loadSettings(key: string): LoopSettings {
  try {
...
export default function LoopViewer({ endpoint, queryKey, settingsKey }: LoopViewerProps) {
  ...
  const [settings, setSettings] = useState<LoopSettings>(() => loadSettings(settingsKey));
      ...
      <main className="flex-1 flex flex-col items-center px-4 py-4 gap-3">
        <div className="w-full max-w-7xl flex items-center gap-3 flex-wrap text-xs text-ink-dim">
          ...
          <label className="inline-flex items-center gap-1.5 rounded-md bg-bg-2 border border-edge px-2 py-1 cursor-pointer">
            <input
              ...
              onChange={(e) => setSettings({ ...settings, rock: e.target.checked })}
        ...
        {current && (
          <div className="text-[11px] text-ink-dim font-mono">
            {new Date(current.timestamp * 1000).toLocaleString(undefined, { hour12: false })}
          </div>
        )}
      </main>
